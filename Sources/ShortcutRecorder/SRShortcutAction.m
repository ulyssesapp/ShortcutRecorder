    ...
    switch (anEventType)
    {
        ...
        case NSEventTypeFlagsChanged:
        {
            ...
            if (keyCode == kVK_Command || keyCode == kVK_RightCommand)
                ...
                os_log(OS_LOG_DEFAULT, "#Error Unexpected key code %hu for the FlagsChanged event", keyCode);
            break;
        }
        default:
...
- (OSStatus)handleEvent:(EventRef)anEvent
{
    ...
    os_activity_initiate("-[SRGlobalShortcutMonitor handleEvent:]", OS_ACTIVITY_FLAG_DETACHED, ^{
        ...
        @synchronized (self->_actions)
        {
            ...
            if (!shortcut)
            {
                os_log(OS_LOG_DEFAULT, "Unregistered hot key with id %u and signature %u", hotKeyID.id, hotKeyID.signature);
                return;
            }
            ...
            switch (GetEventKind(anEvent))
            {
                ...
                case kEventHotKeyReleased:
                    ...
                    break;
                default:
                    os_log(OS_LOG_DEFAULT, "#Error Unexpected key event of type %u", GetEventKind(anEvent));
                    return;
            }
...
- (void)_unregisterHotKeyForShortcutIfNeeded:(SRShortcut *)aShortcut
{
    ...
    os_log(OS_LOG_DEFAULT, "Removing Carbon hot key %u", hotKeyID);
